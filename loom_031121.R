...
approach <- expand.grid(rad_rel_fish = dot_angle$rad_rel_fish, # make all unique combinations of approach angle, size, and speed
  ...
  mutate(dot_numframes = round(start_dist/((fish_len*dot_speed_blps)/fps)), # determine number of frames required for animation (depends on speed)
...
approach <- approach %>%
  ...
  mutate(radius = case_when(frame==1~start_dist,
         ...
         dot_x = radius*cos(rad_rel_fish), # convert stimuli approach angle and radius to cartesian coordinates
...
write.csv(loom_031121_meta,"loom_031121_meta.csv",row.names = F)
